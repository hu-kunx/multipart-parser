...
function parseContentHeader( contents ) {
    ...
    switch ( type[ 0 ] ) {
      case "Content-Disposition":
        for ( let key of params ) {
          const [ k, v ] = key.split( "=" );
          result.disposition[ k.toLocaleLowerCase().trim() ] = v.replace( /"/g, "" )
        }
        break;
      case "Content-Type":
        result.contentType = type[ 1 ].trim();
        if ( params.length > 0 && params[ 0 ].includes( "charset" ) ) {
          result.encoding = params[ 0 ].split( "=" )[ 1 ]
            .replace( /"/g, "" )
            .toLocaleLowerCase()
            .trim();
        }
  ...
  if ( result.disposition.filename || result.disposition[ "filename*" ] || !TEXT_MIME.includes( result.contentType ) ) {
    result.isFile = true;
  }
  return result
}
...
function decode( buf, length, boundary ) {
  ...
  while ( i < length ) {
    ...
    const bodyEndIndex = getFieldBodyEndIndex( buf, endIndex + 4, length, boundaryBuf, boundaryLen );
    let value = '';
    // 是否是文件
    if ( header.isFile ) {
      const path = `${ os.tmpdir() }/node-${ uuid() }`;
      const fsWrite = fs.createWriteStream( "./t.mp3" );
      // +4 是因为 content- 和内容间隔 \r\n\r\n
      // 经过上传 mp3 文件比较 发现 没有多出字节
      fsWrite.write( buf.slice( endIndex + 4, bodyEndIndex ) );
      fsWrite.close();
      value = { path, size: bodyEndIndex - endIndex + 4, filename: header.disposition.filename }
    } else {
...
async function parse( req ,opts) {
  ...
  if ( contentType[ 0 ] !== 'multipart/form-data' || !contentType[ 1 ].includes( "boundary" ) ) {
    return null
  }
